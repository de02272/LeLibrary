...
public class LeServerListeners {
     ...
     * The list of registered connectionListeners
     */
    private static final List<org.die_fabrik.lelib.server.ILeGattListener> gattListeners = new ArrayList<>();
    ...
    public static void onGattConnected(BluetoothDevice device) {
        List<org.die_fabrik.lelib.server.ILeGattListener> listeners = new ArrayList<>();
        synchronized (gattListeners) {
            listeners.addAll(gattListeners);
        }
        for (org.die_fabrik.lelib.server.ILeGattListener listener : listeners) {
     ...
     */
    public static void registerListener(org.die_fabrik.lelib.server.ILeGattListener listener) {
        synchronized (gattListeners) {
